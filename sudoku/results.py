...
def results_to_tsv(run: AutoresearchRun) -> str:
    ...
    """
    lines = ["id\tstatus\tscore\tsteps\ttime_ms\tsolved\tconfig"]
    for exp in run.experiments:
        lines.append(
            f"{exp.experiment_id}\t{exp.status}\t{exp.mean_score:.4f}\t"
...
def summary_table(run: AutoresearchRun) -> str:
    ...
    # Progression table
    lines.append(f"{'#':>3}  {'Status':>10}  {'Score':>7}  {'Steps':>7}  "
    ...
    for exp in run.experiments:
        ...
        lines.append(
            f"{exp.experiment_id:>3}  {exp.status:>10}  {exp.mean_score:>7.4f}  "
